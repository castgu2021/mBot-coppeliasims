...
--********************
    ...
    function DebugLog(text)
		if debugConsoleOn then sim.auxiliaryConsolePrint(console, text.."\n") end
	end
	...
	function float2hex (n)
		...
		elseif (mant == 0.0 and expo == 0) or expo < -0x7E then
		    ...
		    hext[#hext+1] = string.char(sign + math.floor(expo / 0x2), (expo % 0x2) * 0x80 + math.floor(mant / 0x10000),
									math.floor(mant / 0x100) % 0x100,math.floor( mant % 0x100))
	...
	-- Po?alji bajt na serijski port
	function WriteByte(data)
		sim.serialSend(serial, string.char(data))
	end
	
	function Write16bit(b1, b2)
		sim.serialSend(serial, string.char(b1, b2))
	end
	
	function Write32bit(b1, b2, b3, b4)
		sim.serialSend(serial, string.char(b1, b2, b3, b4))
	end
	
	-- Tip podatka u odgovoru se ?alje prije samog podatka: 1 byte 2 float 3 short 4 len+string 5 double
	...
	function RunMotorL(speed)
		-- Mblock u komandi za lijevi motor ?alje invertiranu vrijednost tj. -speed (zbog orijentacije motora u mBot-u)
		DebugLog("Run Motor M1 (left), speed: "..speed)
		if leftWheelJoint == nil then
			DebugLog("Left wheel joint is not defined.")
			return
		end
		sim.setJointTargetVelocity(leftWheelJoint, -speed * motorSpeedCoefficient)
	end
	...
	function RunMotorR(speed)
		...
		sim.setJointTargetVelocity(rightWheelJoint, speed * motorSpeedCoefficient)
	end
	...
	function LedSetColor(port, slot, ledIdx, red, green, blue)
		...
		if ledIdx == 0 or ledIdx == 1 then
			sim.setLightParameters(LED_RGB_right, state, nil, {red, green, blue}, {red, green, blue})
		end
		if ledIdx == 0 or ledIdx == 2 then
			sim.setLightParameters(LED_RGB_left, state, nil, {red, green, blue}, {red, green, blue})
		end
	end
	...
	function SevSegDisplay(value)
		...
		sim.setFloatSignal("SevSegPrecision"..math.floor(sevSegDisplayPrecision), value)
	end
	...
	function ReadLineFollower(port)
		...
		leftSensor = sim.readVisionSensor(leftLineFollower)
		rightSensor = sim.readVisionSensor(rightLineFollower)
		sim.setLightParameters(LED_LineFollower_left, leftSensor, nil, nil, nil)
		sim.setLightParameters(LED_LineFollower_right, rightSensor, nil, nil, nil)
	...
	function ReadUltrasonicSensor(port)
		...
		local result, distance = sim.readProximitySensor(ultrasonicSensor)
		if result > 0 then
	...
	robotIsOn = sim.getScriptSimulationParameter(sim.handle_self,"Robot power",false)
	comPortName = sim.getScriptSimulationParameter(sim.handle_self,"Com port name",false)
	motorSpeedCoefficient = sim.getScriptSimulationParameter(sim.handle_self,"Motor speed coefficient",false)
	motorLeftJointName = sim.getScriptSimulationParameter(sim.handle_self,"Left motor joint name",false)
	motorRightJointName = sim.getScriptSimulationParameter(sim.handle_self,"Right motor joint name",false)
	ultrasonicSensorName = sim.getScriptSimulationParameter(sim.handle_self,"Ultrasonic sensor name",false)
	leftLineFollowerName = sim.getScriptSimulationParameter(sim.handle_self,"Left line follower sensor name",false)
	rightLineFollowerName = sim.getScriptSimulationParameter(sim.handle_self,"Right line follower sensor name",false)
	leftLineFollowerLEDName = sim.getScriptSimulationParameter(sim.handle_self,"Left line follower LED name",false)
	rightLineFollowerLEDName = sim.getScriptSimulationParameter(sim.handle_self,"Right line follower LED name",false)
	sevSegDisplayPrecision = sim.getScriptSimulationParameter(sim.handle_self,"Seven segment display precision",false)
	LED_power_name = sim.getScriptSimulationParameter(sim.handle_self,"LED power name", false)
	LED_battery_name = sim.getScriptSimulationParameter(sim.handle_self,"LED battery name", false)
	LED_battery_power_name = sim.getScriptSimulationParameter(sim.handle_self,"LED battery power name", false)
	LED_RGB_left_name = sim.getScriptSimulationParameter(sim.handle_self,"LED RGB left name", false)
	LED_RGB_right_name = sim.getScriptSimulationParameter(sim.handle_self,"LED RGB right name", false)
	debugConsoleOn = sim.getScriptSimulationParameter(sim.handle_self,"Debug console output",false)
    if debugConsoleOn then 
		console = sim.auxiliaryConsoleOpen("Aux Console", 500, 0x10, {80, 150}, {400, 800}) 
	end
    --*************
	-- Robot parts*
    --*************
	...
	if motorLeftJointName == "" then
		...
		leftWheelJoint = sim.getObjectHandle(motorLeftJointName)
	end
	if motorRightJointName == "" then
		rightWheelJoint = nil
	else
		rightWheelJoint = sim.getObjectHandle(motorRightJointName)
	end
	if ultrasonicSensorName == "" then
		ultrasonicSensor = nil
	else
		ultrasonicSensor = sim.getObjectHandle(ultrasonicSensorName)
	end
	if leftLineFollowerName == "" then
		leftLineFollower = nil
	else
		leftLineFollower = sim.getObjectHandle(leftLineFollowerName)
	end
	if rightLineFollowerName == "" then
		rightLineFollower = nil
	else
		rightLineFollower = sim.getObjectHandle(rightLineFollowerName)
	end
	if leftLineFollowerLEDName == "" then
		LED_LineFollower_left = nil
	else
		LED_LineFollower_left = sim.getObjectHandle(leftLineFollowerLEDName)
	end
	if rightLineFollowerLEDName == "" then
		LED_LineFollower_right = nil
	else
		LED_LineFollower_right = sim.getObjectHandle(rightLineFollowerLEDName)
	end
	if LED_power_name == "" then
		LED_Power = nil
	else
		LED_Power = sim.getObjectHandle(LED_power_name)
	end
	if LED_battery_name == "" then
		LED_Battery = nil
	else
		LED_Battery = sim.getObjectHandle(LED_battery_name)
	end
	if LED_battery_power_name == "" then
		LED_BatteryPower = nil
	else
		LED_BatteryPower = sim.getObjectHandle(LED_battery_power_name)
	end
	if LED_RGB_left_name == "" then
		LED_RGB_left = nil
	else
		LED_RGB_left = sim.getObjectHandle(LED_RGB_left_name)
	end
	if LED_RGB_right_name == "" then
		LED_RGB_right = nil
	else
		LED_RGB_right = sim.getObjectHandle(LED_RGB_right_name)
	end	
	
    
    --**********************
	-- Init Open conections*
    --**********************
    
    comPort = "\\\\.\\"..comPortName  --"\\\\.\\COM2" za Windows
    baudrate = 115200
    if robotIsOn then
		serial = sim.serialOpen(comPort, baudrate)
		if LED_Power ~= nil then sim.setLightParameters(LED_Power, 1, nil, nil, nil) end
		if LED_Battery ~= nil then sim.setLightParameters(LED_Battery, 1, nil, nil, nil) end
		if LED_BatteryPower ~= nil then sim.setLightParameters(LED_BatteryPower, 1, nil, nil, nil) end
	end
    ...
    while true and robotIsOn do  

        
		--buffer = sim.serialRead(serial, 255, false, 0, 100)
        buffer = sim.serialRead(serial,255,false,'',100)
...
if LED_Power ~= nil then sim.setLightParameters(LED_Power, 0, nil, nil, nil) end
if LED_Battery ~= nil then sim.setLightParameters(LED_Battery, 0, nil, nil, nil) end
if LED_BatteryPower ~= nil then sim.setLightParameters(LED_BatteryPower, 0, nil, nil, nil) end
if LED_RGB_left ~= nil then sim.setLightParameters(LED_RGB_left, 0, nil, nil, nil) end 
if LED_RGB_right ~= nil then sim.setLightParameters(LED_RGB_right, 0, nil, nil, nil)end
